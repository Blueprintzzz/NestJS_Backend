...
function mapReview(raw: any): ReviewEntity {
  return { ...raw, images: Array.isArray(raw.images) ? raw.images : [], photos: raw.photos ?? [] };
}
...
export class PrismaReviewRepository implements IReviewRepository {
  ...
  async findById(id: string): Promise<ReviewEntity | null> {
    const r = await this.prisma.review.findUnique({ where: { id }, include: { photos: true } });
  ...
  async findAll(query: ReviewQueryDto): Promise<Pagination<ReviewEntity>> {
    ...
    const [total, rows] = await Promise.all([
      ...
      this.prisma.review.findMany({ where, skip, take: query.limit, orderBy: { createdAt: 'desc' }, include: { photos: true } }),
    ]);
  ...
  async findByPackageId(packageId: string, query: ReviewQueryDto): Promise<{ data: ReviewEntity[]; avgRating: number; total: number }> {
    ...
    const [total, rows, agg] = await Promise.all([
      this.prisma.review.count({ where }),
      this.prisma.review.findMany({ where, take: query.limit, skip: (query.page - 1) * query.limit, include: { photos: true } }),
  ...
  async update(id: string, dto: UpdateReviewDto): Promise<ReviewEntity> {
    const r = await this.prisma.review.update({ where: { id }, data: dto, include: { photos: true } });
    return mapReview(r);
  }
  ...
  async setStatus(id: string, status: string): Promise<ReviewEntity> {
    const r = await this.prisma.review.update({ where: { id }, data: { status: status as ReviewStatus }, include: { photos: true } });
    return mapReview(r);
  }

  async incrementHelpful(id: string): Promise<ReviewEntity> {
    const r = await this.prisma.review.update({ where: { id }, data: { helpful: { increment: 1 } }, include: { photos: true } });
    return mapReview(r);
  }
